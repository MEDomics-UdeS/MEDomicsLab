...
def get_node_content(id: str, json_scene):
    find = False
    json_data = json_scene['json2send'] if 'json2send' in json_scene else json_scene
    for module in json_data['drawflow']:  # We scan all module in scene
        for node_id in json_data['drawflow'][module]['data']:  # We scan all node of each modulef in scene
            if (node_id == str(id)):  # they are "str"
                find = True
                content = json_data['drawflow'][module]['data'][node_id]
