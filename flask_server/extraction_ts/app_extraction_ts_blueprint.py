...
def TSFresh_extraction():
    ...
    df_ts = df_ts.dropna(subset=[json_config["selectedColumns"]["measurementValue"]]).sample(frac=0.1)
    ...
    csv_result_path = os.path.join(str(Path(json_config["csvPath"]).parent.absolute()), "tmp_extraction_result.csv")
    df_extracted_features = df_extracted_features.compute()
