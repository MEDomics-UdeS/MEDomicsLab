      ...
      if (modelDataObject) {
        ...
        if (!modelDataObject.metadata.content) {
          if (!chosenModel.metadata) {
              ...
              customZipFile2Object(chosenModel.path)
                .then((content) => {
                  ...
                  if (content && Object.keys(content).length > 0) {
                    ...
                    let modelData = content.columns.filter((col) => col != content.target)
            ...
            setGlobalData({ ...globalData })
            let modelData = chosenModel.metadata.columns.filter((col) => col != chosenModel.metadata.target)
            checkWarnings(columnsArray, modelData)
          }
        } else {
          console.log("flag1 - false")

          let modelData = modelDataObject.metadata.content.columns.filter((col) => col != modelDataObject.metadata.content.target)  
          checkWarnings(columnsArray, modelData)
        }
