...
export function TabStorage({ tab, layout }) {
	...
	const scroller = useCallback((isDown) => {
	...
	const kickstartingCallback = useCallback((dragging) => {
	...
	const calculateInsertion = useCallback((absoluteY) => {
		const rects = storedTabs.map(json => refs.get(json.id)?.getBoundingClientRect());

		const splits = [rects[0]?.top ?? 0];

		for (let i = 1; i < rects.length; i++) {
			splits.push((rects[i - 1]?.bottom ?? 0 + rects[i]?.top ?? 0) / 2);
		}

		splits.push(rects[rects.length - 1]?.bottom ?? 0);
	...
	const insertionCallback = useCallback((dragging, _, __, y) => {
			...
			{storedTabs.map((stored, i) => (
				<div
					ref={ref => {
						if (ref) {
							if (stored.id) {
								refs.set(stored.id, ref);
							} else {
								console.error('Stored tab is missing an ID');
							}
						} else if (stored.id) {
							refs.delete(stored.id);
						}
					}}
