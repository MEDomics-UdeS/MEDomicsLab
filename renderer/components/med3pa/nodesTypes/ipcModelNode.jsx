...
export default function IPCModelNode({ id, data }) {
  ...
  const renderSettings = (settings) => {
    return (
      <div>
        <div className="mb-3">
          <p className="fw-bold">Model Type:</p>
          <p className="fw-bold mb-0">{settings.model_type}</p>
        </div>
        <hr></hr>
        <div className="mb-3">
        ...
        <Modal.Body>
          {data.internal.settings.model_type && (
            <FlInput
              ...
              name="Model Type"
              settingInfos={{
                ...data.setupParam.possibleSettings.model_type,
                choices: data.setupParam.possibleSettings.model_type.options
              }}
              currentValue={modelType}
              onInputChange={(value) => handleModelTypeChange(value.value)}
            />
          )}
          {data.internal.settings.model_type && (
            <>
