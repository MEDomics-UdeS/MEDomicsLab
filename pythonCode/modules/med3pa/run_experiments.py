...
def _transform_ipc_model_node(node):
    return {
        ...
        "optimize_params": node["settings"].get("optimize_params", None)
    }

def _transform_apc_model_node(node):
    return {
        "model_type": node["settings"].get("model_type", "Decision Tree Regressor"),
        "hyperparameters": node["settings"].get("hyperparameters", None),
        "optimize_params": node["settings"].get("optimize_params", None)
    }
...
def _handle_med3pa_experiment(experiment):
    ...
    ipc_type = ipc_model.get("optimize_params", "RandomForestRegressor")
...
class GoExecScriptRunExperiments(GoExecutionScript):
    ...
    def _custom_process(self, json_config: dict) -> dict:
        ...
        paths = []
        path_to_dir = os.path.join(base_path, "MED3paResults")
        ...
        for index, experiment in enumerate(experiments):
            ...
            saving_path = os.path.join(path_to_dir, experiment_path)
