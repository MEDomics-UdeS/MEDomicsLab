...
class GoExecScriptRunPipelineFromMEDfl(GoExecutionScript):
    ...
    def _custom_process(self, json_config: dict) -> dict:
        ...
        db_manager.create_MEDfl_db(path_to_csv = json_config['flConfig'][0]['masterDatasetNode']['path'])
        ...
        Net_1.create_master_dataset(json_config['flConfig'][0]['masterDatasetNode']['path'])
        ...
        for client in json_config['flConfig'][0]['Network']['clients']:
            hospital = Node(name=client["name"], train=1 if client['type'] == 'Train node' else 0)
            Net_1.add_node(hospital)
            hospital.upload_dataset(client["dataset"]['path'])
        ...
        aggreg_algo = Strategy(json_config['flConfig'][0]['flStrategyNode']['Aggregation algorithm'], 
                                ...
                                min_evaluate_clients=json_config['flConfig'][0]['flStrategyNode']['Minimal used clients for evaluation'],
                                min_available_clients=json_config['flConfig'][0]['flStrategyNode']['Minimal available clients'],
        ...
        server = FlowerServer(global_model,
                                  ...
                                  diff_privacy=True if json_config['flConfig'][0]['Network']['server']['activateDP'] == "Activate" else False,
        ...
        time.sleep(1)
        self.results = {"data" : results ,  "stringFromBackend" : "The configuration is set up"}
