...
json_params = {
"relativeToExtractionType": {
    "selectedColumns": {
        "patientIdentifier": "subject_id",
        "admissionIdentifier": "",
        "admissionTime": "",
        "notesWeight": "",
        "notes": "text",
    ...
    "csvPath": "/Users/medomics/Documents/workspace/DATA/csv/text_notes.csv",
    "filename": "text.csv",
    "dataFolderPath": "/Users/medomics/Documents/workspace/DATA",
    "pageId": "ExtractionTextPage"
}

class GoExecScriptBioBERTExtraction:
    """
    ...
    def __init__(self, json_params: dict):
    ...
    def generate_biobert_notes_embeddings(self, dataframe, frequency, column_id, column_weight, column_text, column_prefix, master_table_compatible, column_admission="", column_admission_time="", column_time=""):
        ...
        if frequency == "Patient":
            ...
            for patient_id in set(dataframe[column_id]):
                ...
                print(now = round(self._progress["now"] + 1/len(set(dataframe[column_id]))*60, 2))
            # Rename columns
            if master_table_compatible:
            ...
            for index, row in dataframe.iterrows():
                ...
                print('now = ' + str(round(self._progress["now"] + 1/len(set(dataframe[column_id]))*60, 2)))
                self._progress["now"] = round(self._progress["now"] + 1/len(set(dataframe[column_id]))*60, 2)
            # Rename columns
            col_number = len(df_notes_embeddings.columns) - 2
            df_notes_embeddings.columns = [column_id, column_time] + [column_prefix + str(i) for i in range(col_number)]
            ...
            for patient_id in set(dataframe[column_id]):
                ...
                while start_date <= last_date:
                    ...
                    end_date += frequency
                # Print progress, starts at 30% and ends at 90%
                print('now =' + str(round(self._progress["now"] + 1/len(set(dataframe[column_id]))*60, 2)))
                self._progress["now"] = round(self._progress["now"] + 1/len(set(dataframe[column_id]))*60, 2)
            # Rename columns
            if master_table_compatible:
    ...
    def _custom_process(self, json_config: dict) -> dict:
        ...
        print('Save extracted features - 90')
...
if __name__ == "__main__":
    script = GoExecScriptBioBERTExtraction(json_params)
    script.start()
